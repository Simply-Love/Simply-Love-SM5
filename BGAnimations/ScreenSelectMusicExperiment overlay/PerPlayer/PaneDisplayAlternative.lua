...
local af =
	...
	SetCommand = function(self)
		local player_score, player_name, player_date
		if PROFILEMAN:IsPersistentProfile(player) and GAMESTATE:GetCurrentSong() then --if there's no song there won't be a hash
			...
			if hash and GetScores(player, hash) then
				...
				player_date = Split(GetScores(player, hash)[1].dateTime)[1]
			else --if we can't generate hashes (malformed SM/DWI/etc) we can't save scores so fallback on profile here
				player_score, player_name, player_date = GetNameAndScoreAndDate(PROFILEMAN:GetProfile(player))
			end
			...
			self:GetChild("PlayerHighScoreDate"):settext(player_date)
		end
	end
}
	...
	LoadFont("Common Normal") ..
		{
			Text = "Steps:",
			InitCommand = function(self)
				self:zoom(zoom_factor):xy(dataX_col1, 150):diffuse(Color.Black):halign(1)
			end
		},
	--  numerical value
	LoadFont("Common Normal") ..
		{
			InitCommand = function(self)
				self:zoom(zoom_factor):xy(labelX_col1, 150):diffuse(Color.Black):halign(0)
			end,
			OnCommand = function(self)
...
--PLAYER PROFILE highscore date
af[#af + 1] =
	LoadFont("Common Normal") ..
	{
		Name = "PlayerHighScoreDate",
		InitCommand = function(self)
			self:xy(highscoreX, 176):zoom(zoom_factor):diffuse(Color.Black):halign(.5)
		end
	}

	...
	if event.type ~= "InputEventType_Release" then
		...
		elseif event.GameButton == "Start" then
			...
			if (type(info[3])=="function") then
				...
				local result = info[3](event.PlayerNumber)

				-- if we're toggling a folder open/close, play the folder sound
				if (info[4] == true) then
					sortmenu:GetChild("toggle_folder_sound"):play()

				-- otherwise, use the `result` of the action to play either start_sound or error_sound
				-- most action_if_chosen functions don't return anything, so `result` will be nil
				-- some explicitly return `false` if an error condition occurs
				else

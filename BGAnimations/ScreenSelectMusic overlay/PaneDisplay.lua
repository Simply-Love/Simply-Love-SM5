...
local PaneItems = {
	-- first row
	{ name=THEME:GetString("RadarCategory","Taps"),  sig=0, rc='RadarCategory_TapsAndHolds'},
	{ name=THEME:GetString("RadarCategory","Mines"), sig=0, rc='RadarCategory_Mines'},
	-- { name=THEME:GetString("ScreenSelectMusic","NPS") },

	-- second row
	{ name=THEME:GetString("RadarCategory","Jumps"), sig=0, rc='RadarCategory_Jumps'},
	{ name=THEME:GetString("RadarCategory","Hands"), sig=0, rc='RadarCategory_Hands'},
	-- { name=THEME:GetString("RadarCategory","Lifts"), rc='RadarCategory_Lifts'},

	-- third row
	{ name=THEME:GetString("RadarCategory","Holds"), sig=0, rc='RadarCategory_Holds'},
	{ name=THEME:GetString("RadarCategory","Rolls"), sig=0, rc='RadarCategory_Rolls'},
	-- { name=THEME:GetString("RadarCategory","Fakes"), rc='RadarCategory_Fakes'},
}

-- Will try to show these, if available (i.e. dance-single mode). If not, will fall
-- back to the radar-style PaneItems above. Can also be disabled in settings.
--                                          ^^ TODO make this be true.
-- NB: for sigbovik-style items, `rc` becomes the index into `SIGBOVIKSongStats`
local PaneItemsSIGBOVIKEdition = {
       -- first row
       { name=THEME:GetString("RadarCategory","Taps"),  sig=0, rc='RadarCategory_TapsAndHolds' },
       -- TODO FIXME: THEME:GetString("SIGBOVIK","...") appears to crash...
       { name="Crossovers",   sig=1, rc=1 },

       -- second row
       { name=THEME:GetString("RadarCategory","Jumps"), sig=0, rc='RadarCategory_Jumps' },
       { name="Footswitches", sig=1, rc=2 },

       -- third row
       { name="Brackets",     sig=1, rc=3 },
       { name="Sideswitches", sig=1, rc=4 },
       -- (TODO: add an option in user prefs to swap out sideswitches for jacks?
}

local SIGBOVIKSongStats = { 111, 222, 333, 444 } --{ 'X'=111, 'F'=222, 'S'=333, 'B'=444 }
local SIGBOVIKSongStatsLastUpdated = {
	filename="",
	difficulty="",
	description="", -- disambiguates Edits
}

function UpdateSIGBOVIKSongStatsIfNecessary(StepsOrTrail)
	local filename = StepsOrTrail:GetFilename()
	if not filename then return end
	local difficulty = StepsOrTrail:GetDifficulty()
	if not difficulty then return end
	local description = StepsOrTrail:GetDescription()
	if not description then return end

	if filename ~= SIGBOVIKSongStatsLastUpdated.filename or difficulty ~= SIGBOVIKSongStatsLastUpdated.difficulty or description ~= SIGBOVIKSongStatsLastUpdated.description then
		-- needs updated
		local xo, fs, ss, _jk, br = GetSongStatsSIGBOVIKEdition(StepsOrTrail)
		SIGBOVIKSongStats[1] = xo
		SIGBOVIKSongStats[2] = fs
		SIGBOVIKSongStats[3] = br
		SIGBOVIKSongStats[4] = ss

		SIGBOVIKSongStatsLastUpdated.filename = filename
		SIGBOVIKSongStatsLastUpdated.difficulty = difficulty
		SIGBOVIKSongStatsLastUpdated.description = description
	end
end
...
for player in ivalues(PlayerNumber) do
	...
	-- decide which type of song stats to show (radar style or sigbovik crossoveriness style)
	local PaneItemsDuJour
	if false then -- TODO make configurable
		   PaneItemsDuJour = PaneItems
	else
		   PaneItemsDuJour = PaneItemsSIGBOVIKEdition
	end
	
	for i, item in ipairs(PaneItemsDuJour) do
		...
		af2[#af2+1] = Def.ActorFrame{
			...
			LoadFont("Common Normal")..{
				...
				SetCommand=function(self)
					...
					if item.sig == 1 then
						-- since this callback runs once per *stat*, we wanna avoid
						-- running the expensive O(n) alg multiple times on the same song
						UpdateSIGBOVIKSongStatsIfNecessary(StepsOrTrail)

						-- SIGBOVIK-style song stat (table value is guaranteed to exist)
						self:settext(SIGBOVIKSongStats[item.rc])
					elseif item.rc then
